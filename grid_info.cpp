...
//! This function outputs hydro evolution file, CAUSALITY ANALYSIS
// Parte para os cortes no music usados para o estudo da causalidade.
void Cell_info::OutputEvolutionDataXYEta_causality(SCGrid &arena, double tau) {
    const string out_name_W_xyeta =
                        "evolution_Wmunu.dat";
    const string out_name_bulkpi_xyeta = "evolution_bulk_pressure.dat";
    string out_open_mode;
    FILE *out_file_W_xyeta      = NULL;
    FILE *out_file_bulkpi_xyeta = NULL;
    ...
    for (int ieta = 0; ieta < arena.nEta(); ieta += n_skip_eta) {
        ...
        for (int iy = 0; iy < arena.nY(); iy += n_skip_y) {
            for (int ix = 0; ix < arena.nX(); ix += n_skip_x) {
                ...
                double eta = 0.11 + 1.6*(T_local*hbarc - 0.154)*pow((T_local*hbarc/0.154), -0.29);
                double zeta = 0.032/(1+pow((((T_local*hbarc)-0.175)/0.024), 2));
                ...
                if (DATA.outputBinaryEvolution == 0) {
                    if ((DATA.viscosity_flag == 1)&&(DATA.turn_on_shear)) {
                        fprintf(out_file_W_xyeta,
                                ...
                                Wtautau, Wtaux, Wtauy, Wtaueta, Wxx, Wxy,
                                Wxeta, Wyy, Wyeta, Wetaeta, p_local, cs2_local, enthropy, e_local, T_local*hbarc, eta, zeta);
                    }
                } else {
                    if (DATA.viscosity_flag == 1) {
                        if (DATA.turn_on_shear == 1) {
					      ...
					      static_cast<float>(T_local*hbarc),
                            ...
                            fwrite(array2, sizeof(float), 17,
                                   out_file_W_xyeta);
                        }
                        if (DATA.turn_on_bulk == 1) {
                            float array1[] = {static_cast<float>(bulk_Pi)};
                            fwrite(array1, sizeof(float), 1,
                                   out_file_bulkpi_xyeta);
                        }
                    }
                }
...
void Cell_info::get_maximum_energy_density(SCGrid &arena, double tau) {
    double eps_max  = 0.0;
    double rhob_max = 0.0;
    double T_max    = 0.0;
